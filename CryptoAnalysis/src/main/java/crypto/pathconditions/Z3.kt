...
fun Z3Solver.encode(expr: JExpression, expectedSort: Sort): Expr =
    when (expr) {
        ...
        is JAdd -> {
            val expectedOpSort = encodeType(expr.left.type)
            context.mkAddOrFPAdd(
                encode(expr.left, expectedOpSort),
                encode(expr.right, expectedOpSort)
            )
        }
        is JSubtract -> {
            val expectedOpSort = encodeType(expr.left.type)
            context.mkSubOrFPSub(
                encode(expr.left, expectedOpSort),
                encode(expr.right, expectedOpSort)
            )
        }
        ...
        is JRemainder -> {
            val expectedOpSort = encodeType(expr.left.type)
            context.mkRem(
                encode(expr.left, expectedOpSort) as IntExpr,
                encode(expr.right, expectedOpSort) as IntExpr
            )
        }
        is JCondition -> {
...
fun Z3Solver.decode(expr: Expr): JExpression = when {
    expr.isAnd -> JAnd(decode(expr.args[0]), decode(expr.args[1]))
    expr.isOr -> JOr(decode(expr.args[0]), decode(expr.args[1]))
    expr.isEq -> {
        val op1 = decode(expr.args[0])
        val op2 = decode(expr.args[1])
        JCondition(op1, op2, JEquals)
    }
    expr.isNot && expr.args[0].isEq -> {
        val op1 = decode(expr.args[0].args[0])
        val op2 = decode(expr.args[0].args[1])
        JCondition(op1, op2, JNotEquals)
    }
    expr.isGT || expr.isFPGT -> {
        val op1 = decode(expr.args[0])
        val op2 = decode(expr.args[1])
        JCondition(op1, op2, JGreaterThan)
    }
    expr.isGE || expr.isFPGE -> {
        val op1 = decode(expr.args[0])
        val op2 = decode(expr.args[1])
        JCondition(op1, op2, JGreaterOrEqual)
    }
    expr.isLT || expr.isFPLT -> {
        val op1 = decode(expr.args[0])
        val op2 = decode(expr.args[1])
        JCondition(op1, op2, JLessThan)
    }
    expr.isLE || expr.isFPLE -> {
        val op1 = decode(expr.args[0])
        val op2 = decode(expr.args[1])
        JCondition(op1, op2, JLessOrEqual)
    }
